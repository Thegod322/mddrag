...
1. Obsidian Canvas-based modular documentation
2. External documentation (libraries, frameworks, tools)
"""
...
async def handle_list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Search in documentation indexed from Libraries directory",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    # External documentation tools
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"Successfully indexed {result['indexed_count']} documents from '{doc_name}' v{version}"
            )]
        ...
        try:
            if not external_docs_engine:
                external_docs_engine = ExternalDocsEngine()
            results = await external_docs_engine.search(
                query=query,
                limit=limit
            )
            if not results:
                return [TextContent(
                    type="text",
                    text="No relevant documentation found. Make sure you have loaded some documentation first."
                )]
            formatted_results = []
            for i, result in enumerate(results, 1):
                formatted_results.append(f"Result {i}:")
